<?php
/**
 * Narnoo Distributor - Albums table.
 **/
class Narnoo_Distributor_Albums_Table extends WP_List_Table {		
	...
	function __construct( $args = array() ) {
		...
		if ( ! is_null( $list ) ) {
			...
			$this->select_album_html_script = Narnoo_Distributor_Helper::get_album_select_html_script( $list->distributor_albums, $total_pages, $this->current_album_page, $this->current_album_name );
		}		
	...
	function column_thumbnail_image( $item ) {    
		$actions = array(
			'remove'    	=> sprintf( 
									...
									__( 'Remove from album', NARNOO_DISTRIBUTOR_I18N_DOMAIN ) 
								),
			'download'    	=> sprintf( 
									'<a href="?%s">%s</a>', 
									build_query( 
	...
	function get_bulk_actions() {
		$actions = array(
			'remove'    	=> __( 'Remove from album', NARNOO_DISTRIBUTOR_I18N_DOMAIN ),
	...
	function process_action() {
		...
		if ( isset( $_REQUEST['back'] ) || isset( $_REQUEST['album_select_button'] ) ) {
			return true;
		}
		...
		if ( false !== $action ) {
			if ( $action === 'create' ) {
				?>
				<h3><?php _e( 'Create album', NARNOO_DISTRIBUTOR_I18N_DOMAIN ); ?></h3>
				<table class="form-table">
					<tr>
						<th><?php _e( "Please key in a new album name:", NARNOO_DISTRIBUTOR_I18N_DOMAIN ); ?></th>
			...
			if ( $action === 'do_create' ) {
				...
				?>
				<h3><?php _e( 'Create album', NARNOO_DISTRIBUTOR_I18N_DOMAIN ); ?></h3>
				<p><?php echo sprintf( __( "Creating the following album:", NARNOO_DISTRIBUTOR_I18N_DOMAIN ) ) . ' ' . esc_html( $new_album_name ); ?></p>
				<ol>
				<?php
				Narnoo_Distributor_Helper::print_media_ajax_script_body( 'unknown', 'albumCreate', array( $new_album_name ) );
				?>
				</ol>
				<?php 
				Narnoo_Distributor_Helper::print_ajax_script_footer( 1, __( 'Back to albums', NARNOO_DISTRIBUTOR_I18N_DOMAIN ) );
			...
			switch ( $action ) {
				...
				case 'remove':
					...
					$album_name = $this->current_album_name;
					if ( empty( $album_name ) ) {
						Narnoo_Distributor_Helper::show_error( __( 'Unspecified album name. Action cancelled.' ), NARNOO_DISTRIBUTOR_MEDIA_I18N_DOMAIN );
						return true;
					}
					?>
					<h3><?php _e( 'Confirm remove from album', NARNOO_DISTRIBUTOR_MEDIA_I18N_DOMAIN ); ?></h3>
					<p><?php echo sprintf( __( "Please confirm removal of the following %d image(s) from the album '%s' (ID %d):", NARNOO_DISTRIBUTOR_MEDIA_I18N_DOMAIN ), $num_ids, esc_html( $album_name ), $album_id ); ?></p>
					...
					foreach ( $image_ids as $id ) { 
						...
						<li><span>Image ID: <?php echo $id; ?></span><span><img style="vertical-align: middle; padding-left: 20px;" src="<?php echo ( isset( $_REQUEST[ 'url' . $id ] ) ? esc_attr( $_REQUEST[ 'url' . $id ] ) : '' ); ?>" /></span></li>
						<?php 
					} 
					?>
					</ol>
				...
				case 'do_remove':
					...
					$album_name = $this->current_album_name;
					if ( empty( $album_name ) ) {
						Narnoo_Distributor_Helper::show_error( __( 'Unspecified album name. Action cancelled.' ), NARNOO_DISTRIBUTOR_I18N_DOMAIN );
						return true;
					}
					?>
					<h3><?php _e( 'Remove from album' ); ?></h3>
					<p><?php echo sprintf( __( "Removing the following %s image(s) from album '%s' (ID %d):", NARNOO_DISTRIBUTOR_I18N_DOMAIN ), $num_ids, $album_name, $album_id ); ?></p>
					<ol>
					<?php
					foreach( $image_ids as $id ) {
						Narnoo_Distributor_Helper::print_media_ajax_script_body( $id, 'albumRemoveImage', array( $id, $album_id ) );
					}
					?>
					</ol>
					<?php 
	...
	function get_current_page_data() {
		...
		if ( ! is_null( $request ) ) {
			try {
				$list = $request->getAlbumImages( $current_album_name, $current_page );
		...
		if ( ! is_null( $list ) ) {
			...
			foreach ( $list->distributor_albums_images as $image ) {
	...
	static function add_screen_options() {
		...
		$narnoo_distributor_albums_table = new Narnoo_Distributor_Albums_Table();
	}
}    